...
describe("Deal Resolver", () => {
  ...
  test("super admin can fetch any investors data", async () => {
    const { _id } = await apolloServer.db.users.findOne({
  ...
  test("fund admin can fetch an investor that is part of their fund", async () => {
    ...
    const { _id } = await db.users.findOne({
      email: "investor@allocations.com",
    });
  ...
  describe("fund admin", () => {
    ...
    test("super admin can see all funds", async () => {
      const {
        ...
        email: "newSuperAdmin@allocations.com",
        admin: true,
      });
      ...
      const {
        data: {
          investor: { organizations_admin },
        },
      } = await query(FUND_ADMINS);
